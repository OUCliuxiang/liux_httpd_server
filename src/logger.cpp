...
vector<Logger::ptr> Logger::_instances(LEVEL_SIZE, nullptr);
BlockingQueue<string> Logger::_queue;

Logger::ptr Logger::instance(int level) {
    assert(level >= 0 && level <= LEVEL_SIZE);
    if (Logger::_instances[level] == nullptr) 
        Logger::_instances[level] = Logger::ptr(new Logger(level));
...
void Logger::thread_func() {
    // ofstream _output_file("/root/liux_httpd_server/log/logtmp.log", ios::out );
    while(true) {
